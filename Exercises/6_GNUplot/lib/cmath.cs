...
public static double  sinh(double x) {return (exp(x)/2-exp(-x)/2);}
public static complex sinh(complex z) { return ((exp(z) / 2 - exp(-z) / 2));}


public static double cosh(double x)     { return (exp(-x) / 2 + exp(x) / 2);}
public static complex cosh(complex z)   { return (exp(-z) / 2 + exp(z) / 2);}

public static complex sqrt(complex z)   {
        return new complex(sqrt(abs(z)) * cos(arg(z)/2),
                           sqrt(abs(z)) * sin(arg(z)/2)); }
