...
public static vector rk23(
	...
	double b,			//intergrate to this point
	double acc=1e-3, 	//absoulute accurrecy
	double eps=1e-3, 	//relative accurrecy
...
public static vector driver(Func<double,vector,vector> F,
	...
	List<double> xlist,	List<vector> ylist,	int limit,
...
{//ODE Driver
	...
	do{
		if(a>=b) return ya;
		if(nsteps>limit){
		Error.WriteLine($"ode.driver: nsteps>{limit}");
		return ya;}
		if(a+h>b) h=b-a;
		...
		else{h=hnew; Error.WriteLine($"driver: badstep at {a}"); }
	}	while(true);
}
}
